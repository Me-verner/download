def _parse_config_string(self, config_string: str):
        """Parse configuration from string format: ip,password,bottoken,admin1,admin2,..."""
        try:
            ...
            self.iranian_user = "root"  # Default
            self.ssh_port = 22  # Default
            ...
            print(f"✅ Configuration loaded: {self.iranian_ip}, Telegram: {'✅' if self.telegram_token else '❌'}")
    ...
    def prompt_for_config(self):
        """Prompt user for configuration in single chain format"""
...
def main():
    ...
    # Check for config string in environment or arguments
    config_string = None
    
    # Check if config string is provided as first argument
    if len(sys.argv) > 1 and ',' in sys.argv[1]:
        config_string = sys.argv[1]
        # Remove config from args and shift
    ...
    # Create manager with config string if provided
    manager = CompleteSSHTunnelSOCKS5Manager(config_string)
    
    # If no config was provided, prompt for it
...
[bold]Usage:[/bold]
  python3 {sys.argv[0] if sys.argv else 'complete_ssh_socks5_manager.py'} [CONFIG] [COMMAND] [PORT1] [PORT2] ...

[bold]Configuration (Interactive):[/bold]
  Just run the script and it will prompt you for configuration:
  [dim]python3 complete_ssh_socks5_manager.py[/dim]

[bold]Configuration (Single Line):[/bold]
  [cyan]server_ip,ssh_password,telegram_bot_token,admin_id1,admin_id2,...[/cyan]
  
  Examples:
  [dim]python3 complete_ssh_socks5_manager.py 94.182.150.195,mypass123[/dim]
  [dim]python3 complete_ssh_socks5_manager.py 94.182.150.195,mypass123,7624988827:AAGb36v5sUG,434418436[/dim]
...
[bold]Commands:[/bold]
  ...
  [cyan]config[/cyan]    - Show configuration format help
...
[bold]What You Get:[/bold]
  ...
  ✅ Single-line configuration support
...
[bold]Quick Start Examples:[/bold]
  ...
  [dim]# Quick start with config[/dim]
  [dim]python3 complete_ssh_socks5_manager.py 94.182.150.195,mypassword[/dim]
  
  [dim]# With Telegram bot[/dim]
  [dim]python3 complete_ssh_socks5_manager.py 94.182.150.195,mypass,bottoken,admin1,admin2[/dim]
  
  [dim]# Custom ports[/dim]
  [dim]python3 complete_ssh_socks5_manager.py 94.182.150.195,mypass start 1080 1081 1082 1083[/dim]
...
[bold blue]Client Configuration:[/bold blue]
  ...
  • Host: [yellow]YOUR_SERVER_IP[/yellow] (your Iranian server)
...
class CompleteSSHTunnelSOCKS5Manager:
    ...
    def __init__(self, config_string: str = None):
        # Parse configuration from string if provided
        if config_string:
            self._parse_config_string(config_string)
        else:
            # Default configuration - will be prompted for
